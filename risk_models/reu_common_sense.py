...
interventions = ["Nothing", "AMF", "CF campaign", "Shrimp - Ammonia", "Shrimp - Stunning", "X-Risk - Scenario 1", "X-Risk - Scenario 2", \
                 "X-Risk - Scenario 3", "X-Risk - Scenario 4", "X-Risk - Scenario 5", "X-Risk - Scenario 6", \
...
def get_daly_burden_by_state(states_dict, state, daly_burden_by_harm, to_print=False):
    ...
    # add in malaria because we know it's a problem
...
def get_dalys_saved_spend_money(intervention, chicken_sent, shrimp_sent, amf_dalys_per_1000, hens_dalys_per_1000, \
    ...
    if intervention == "Nothing":
        ...
        if chicken_sent == 1:
            ...
            effect = np.zeros(N)
    elif intervention == "Shrimp - Stunning":
        if shrimp_sent == 1:
            ...
            effect = np.zeros(N)
    elif intervention == "Shrimp - Ammonia":
        if shrimp_sent == 1:
...
def get_lottery_for_action(intervention, daly_burden_by_harm, states_dict, daly_burden_dict, joint_prob_dict, events_prob_dict, \
    ...
    amf_dalys_per_1000, hens_dalys_per_1000, shrimp_slaughter_dalys_per_1000, shrimp_nh3_dalys_per_1000, \
        s1_xrisk_reduced_per_bn, s2_xrisk_reduced_per_bn, s3_xrisk_reduced_per_bn, s4_xrisk_reduced_per_bn, \
        s5_xrisk_reduced_per_bn, s6_xrisk_reduced_per_bn, s7_xrisk_reduced_per_bn, s8_xrisk_reduced_per_bn, \
        s9_xrisk_reduced_per_bn, s10_xrisk_reduced_per_bn, s11_xrisk_reduced_per_bn, s12_xrisk_reduced_per_bn = cea_inputs
    
    lottery = {}
    lottery_old = {}

    if intervention in ["Nothing", "AMF", "CF campaign", "Shrimp - Stunning", "Shrimp - Ammonia"]:
        for state in states_dict.keys():
            ...
            change_dalys = get_dalys_saved_spend_money(intervention, chicken_sent, shrimp_sent, amf_dalys_per_1000, \
                                                     hens_dalys_per_1000, shrimp_slaughter_dalys_per_1000, \
                                                    shrimp_nh3_dalys_per_1000, daly_burden_by_harm)
    ...
    elif intervention in ["X-Risk - Scenario 1", "X-Risk - Scenario 2", \
                 "X-Risk - Scenario 3", "X-Risk - Scenario 4", "X-Risk - Scenario 5", "X-Risk - Scenario 6", \
                "X-Risk - Scenario 7", "X-Risk - Scenario 8", "X-Risk - Scenario 9", "X-Risk - Scenario 10", \
                "X-Risk - Scenario 11", "X-Risk - Scenario 12"]:
        ...
        if intervention == "X-Risk - Scenario 1":
            decrease_xrisk = get_decrease_xrisk_money(s1_xrisk_reduced_per_bn, to_print)
        elif intervention == "X-Risk - Scenario 2":
            decrease_xrisk = get_decrease_xrisk_money(s2_xrisk_reduced_per_bn, to_print)
        elif intervention == "X-Risk - Scenario 3":
            decrease_xrisk = get_decrease_xrisk_money(s3_xrisk_reduced_per_bn, to_print)
        elif intervention == "X-Risk - Scenario 4":
            decrease_xrisk = get_decrease_xrisk_money(s4_xrisk_reduced_per_bn, to_print)
        elif intervention == "X-Risk - Scenario 5":
            decrease_xrisk = get_decrease_xrisk_money(s5_xrisk_reduced_per_bn, to_print)
        elif intervention == "X-Risk - Scenario 6":
            decrease_xrisk = get_decrease_xrisk_money(s6_xrisk_reduced_per_bn, to_print)
        elif intervention == "X-Risk - Scenario 7":
            decrease_xrisk = get_decrease_xrisk_money(s7_xrisk_reduced_per_bn, to_print)
        elif intervention == "X-Risk - Scenario 8":
            decrease_xrisk = get_decrease_xrisk_money(s8_xrisk_reduced_per_bn, to_print)
        elif intervention == "X-Risk - Scenario 9":
            decrease_xrisk = get_decrease_xrisk_money(s9_xrisk_reduced_per_bn, to_print)
        elif intervention == "X-Risk - Scenario 10":
            decrease_xrisk = get_decrease_xrisk_money(s10_xrisk_reduced_per_bn, to_print)
        elif intervention == "X-Risk - Scenario 11":
            decrease_xrisk = get_decrease_xrisk_money(s11_xrisk_reduced_per_bn, to_print)
        elif intervention == "X-Risk - Scenario 12":
            decrease_xrisk = get_decrease_xrisk_money(s12_xrisk_reduced_per_bn, to_print)
...
def get_ratio_of_change_in_reu_by_cause(changes_in_reu, output_name):
    ...
    causes = ["AMF", "CF campaign", "Shrimp - Stunning", "Shrimp - Ammonia", "X-Risk - Scenario 1", "X-Risk - Scenario 2", \
                 "X-Risk - Scenario 3", "X-Risk - Scenario 4", "X-Risk - Scenario 5", "X-Risk - Scenario 6", \
                "X-Risk - Scenario 7", "X-Risk - Scenario 8", "X-Risk - Scenario 9", "X-Risk - Scenario 10", \
                "X-Risk - Scenario 11", "X-Risk - Scenario 12"]
    ...
    df_ratio_of_change_in_reu_by_cause.to_csv(os.path.join('common_sense_results', output_name))
...
def get_proportion_time_best_action(reus, output_name):
    ...
    df_proportion_times_best_action.to_csv(os.path.join('common_sense_results', output_name))
